/*
 * @Description: 第9章 9.2 顺序统计量的随机选择算法
 ...
 * @LastEditTime: 2020-03-12 11:51:20
 * @LastEditors:
*/
...
func (a *RandomizedSelect) randomized_select(s []int, begin, end, rank int, compare SortCompareFunc) (int, error) {
	...
	if rank >= end {
		return 0, errors.New("rank must be no more than end")
	}
	...
	random := rand.Intn(size - 1)
	partitioned_iter := a.partition(s, begin, end, begin+random, compare) //随机划分
	...
	if distance == rank { //找到了该排位的数
		...
		return a.randomized_select(s, partitioned_iter+1, end, rank, compare)
 ...
 * @param s:数据源
 * @param rank:第rank小,rank从0到size-1
